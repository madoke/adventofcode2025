...
export class Day8Puzzle1 implements PuzzleInterface {
	...
	private computeCircuits(maxConnections: number): BigNumber {
		const circuits: Set<string>[] = [];
		// let connectionsRemaining = maxConnections;
		// let i = 0;
		...
		// while (connectionsRemaining > 0) {
			...
			if (!connection) {
				throw new Error("Could not find closest point");
			}
			...
			if (!fitsExistingCircuit) {
				console.log(`Creating new circuit with ${pointA} and ${pointB}`);
				circuits.push(new Set([pointA, pointB]));
				// connectionsRemaining -= 1;
			}

			if (indexToMergeWith && indexesToMerge.length > 0) {
		...
		const sortedCircuits = circuits.sort((b, a) => a.size - b.size);

		for(const c of sortedCircuits) {
			console.log(c.size);
		}

		const biggest3 = sortedCircuits.slice(0, 3);
		return new BigNumber(biggest3.reduce((prev, item) => item.size * prev, 1));
	}
	...
	public async run(inputFilePath: string): Promise<BigNumber> {
		...
		const totalSum = this.computeCircuits(1000);
